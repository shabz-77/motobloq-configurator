...
class WebSocketTransport extends events_1.EventEmitter {
    ...
    connect(connectionURL) {
        Logger_1.Logger.Info(connectionURL);
        try {
    ...
    handleOnMessageBinary(event) {
        ...
        event.data
            ...
            .catch((error) => {
            Logger_1.Logger.Error(`Failed to parse binary blob from websocket, reason: ${error.message}`);
        });
    }
    /**
    ...
    handleOnMessage(event) {
        ...
        try {
            ...
            if (e instanceof Error) {
                Logger_1.Logger.Error(`Error parsing message string ${event.data}.\n${e.message}`);
            }
            else {
                Logger_1.Logger.Error(`Unknown error while parsing message data in handleOnMessage`);
            }
            return;
        }
        if (this.onMessage) {
     ...
     */
    handleOnOpen() {
        Logger_1.Logger.Info('Connected to the signalling server via WebSocket');
        this.emit('open');
    }
    /**
     ...
     */
    handleOnClose(event) {
        Logger_1.Logger.Info('Disconnected to the signalling server via WebSocket: ' +
